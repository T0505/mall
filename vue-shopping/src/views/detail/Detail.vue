<template>
  <div class="area flex between center">
    <ul v-if="goods">
      ...
      <li class="li-click-scroll white pointer"
        @click="active + this.goods.storeInfo.sliderImageArr.length - 1 && active--">
        <el-icon>
          <ArrowDownBold />
        </el-icon>
      </li>
    ...
    <ul v-if="goods && now" class="ul-goods-text left auto">
      ...
      <li class="li-bottom-function flex">
        ...
        <el-button color="#ff7800" @click="goBuy">购物</el-button>
        <div class="div-collect center" :style="goods.storeInfo.userCollect ? {
          backgroundColor: '#ff7800',
          border: 'none',
          color: 'white',
        } : {}">
          <div><el-icon style="font-size: 14px;">
              <Star />
            </el-icon></div>
          <div>收藏</div>
        </div>
  ...
  <div class="area">
    ...
    <div class="flex div-collect-detail between">
      <div v-if="select" class="flex column auto">
            ...
            <div class="flex column evenly">
              ...
              <ul class="flex">
                <li v-for="(v, o) of item.picturesArr" :key="o">
                  <img style="margin-right: 10px;" :src="$url.substring(0, $url.length - 1) + v" width="80" alt="">
                </li>
              </ul>
        ...
        <a ref="a" href="#mao" />
        <el-pagination class="pagination-detail-comment" background layout="prev, pager, next, jumper"
          :total="count[point] || 1" :default-page-size="5" @current-change="change" />
      </div>
